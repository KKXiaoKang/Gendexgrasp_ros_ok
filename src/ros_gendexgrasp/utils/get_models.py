...
def get_handmodel(robot, batch_size, device, hand_scale=1.2):
...
if __name__ == '__main__':
    ...
    robot_name = 'lejuhand'  # barrett dof #8
    init_opt_q = torch.zeros(1, 9 + 15, device='cuda')
    ...
    # init_opt_q[:, 9:].copy_(hand_model.revolute_joints_q_lower.repeat(1, 1) + 0.1)
    init_opt_q[:, 9:].copy_((hand_model.revolute_joints_q_lower.repeat(1, 1) + hand_model.revolute_joints_q_upper.repeat(1, 1)) / 3)
    ...
    vis_data = hand_model.get_plotly_data(q=init_opt_q, color='pink', opacity=1.0)
