...
from grasp_ik_arm_traj.srv import excuteCumotionTraj,excuteCumotionTrajResponse, excuteCumotionTrajRequest # cumotion避障轨迹excute执行调用
...
OBJECT_DETECTED_FLAG = False # 物体检测是否完成
GRASP_IN_PROGRESS_FLAG = False # 是否正在执行抓取流程
IF_FIRST_GRASP_FLAG = True # 是否第一次抓取
# 添加一个全局计数器变量
GRASP_COUNT = 0
...
def object_detection_callback(msg):
    """
    回调函数，用于处理YOLO检测到的物体数据。
    """
    global OBJECT_DETECTED_FLAG
    if msg.detections:  # 检测到物体
        # rospy.loginfo("物体被检测到！")
        OBJECT_DETECTED_FLAG = True   # 更新物体检测状态
    else:
        # rospy.loginfo("没有检测到物体")
        OBJECT_DETECTED_FLAG = False  # 更新物体检测状态

rospy.Subscriber("/object_yolo_tf2_torso_result", Detection2DArray, object_detection_callback)
...
global_robot_real_no_zero_init_state_joint = JointState( 
    header=Header(stamp=rospy.Time(0), frame_id='torso'),
    name=[
        'zarm_l1_joint', 'zarm_l2_joint', 'zarm_l3_joint', 'zarm_l4_joint', 'zarm_l5_joint', 'zarm_l6_joint', 'zarm_l7_joint'
    ],
...
# 调取excute执行轨迹
excute_plan_service_name = "/excute_plan_traj_cumotion"
rospy.wait_for_service(excute_plan_service_name)
excute_plan_traj_client = rospy.ServiceProxy(
    excute_plan_service_name,  excuteCumotionTraj
)
...
## ------------------------------------------ ROS服务函数 -------------------------------------------------- ## 
def call_execute_plan_traj(id):
    """
    调用/excute_plan_traj_cumotion服务来执行轨迹
    """
    global excute_plan_traj_client
    try:
        # 强制将id转换为整数类型
        id = int(id)
        
        # 创建请求消息
        request = excuteCumotionTrajRequest()
        request.id = id  # 设置请求的ID
        
        # 调用服务并获取响应
        response = excute_plan_traj_client(request)
        
        # 检查服务是否成功执行
        if response.success:
            rospy.loginfo(f"Successfully executed the plan with ID {id}.")
        else:
            rospy.logwarn(f"Execution failed for ID {id}.")
    
    except rospy.ServiceException as e:
        rospy.logerr(f"Service call failed: {e}")
    except ValueError as ve:
        rospy.logerr(f"Invalid ID value: {ve}")
...
def cuMotion_planning_thread():
    ...
    rospy.loginfo(" ------cuMotion轨迹回调 | back状态轨迹回调 |  call_cumotion_service(True, global_robot_new_real_target_state_joint, global_robot_real_zero_state_joint) -------- ")
...
def gendexgrasp():
    ...
    global global_robot_real_init_state_joint
    global global_robot_real_no_zero_init_state_joint
    global global_robot_real_target_state_joint
    global global_robot_real_current_state_joint
    global USE_SIM_FLAG
    global CUMOTION_FINSHED_FLAG
    ...
    msg.joint_data = [-18.0, -12.0] # 往左一度触发更新 准备位置为[-15.0, -12.0]
    print("msg.joint_data: ", msg.joint_data)
    robot_head_pub.publish(msg)
    time.sleep(0.2)

    msg = robotHeadMotionData()
    msg.joint_data = [-15.0, -12.0] # 往左一度触发更新 准备位置为[-15.0, -12.0]
    print("msg.joint_data: ", msg.joint_data)
    robot_head_pub.publish(msg)
    time.sleep(0.2)
    ...
    if IF_USE_PRE_POSE_FLAG: # 如果要使用预定准备的姿态，则需要来到这个位置
        call_cumotion_service(True, global_robot_real_no_zero_init_state_joint, global_robot_real_target_state_joint)
    ...
    # （4） 根据监听/kuavo_cumotion_avoid_joint_target', JointState的结果 | 和手臂此时状态的结果 | 构建cumotion避障轨迹客户端 | 
    # time.sleep(1)
    call_cumotion_service(True, global_robot_real_no_zero_init_state_joint, global_robot_real_target_state_joint) # 第一次调用规划状态
    rospy.loginfo(" ----------------------- cuMotion避障轨迹规划 | go 状态 ------------------------- ")
    rospy.loginfo(f"global_robot_real_target_state_joint.position: {global_robot_real_target_state_joint.position}")
    rospy.loginfo(f"global_robot_new_real_target_state_joint.position: {global_robot_new_real_target_state_joint.position}")
    ...
    while not CUMOTION_FINSHED_FLAG:
        ...
        rospy.loginfo(f"查询cumotion轨迹规划完成状态: {CUMOTION_FINSHED_FLAG}")
        CUMOTION_FINSHED_FLAG = call_cumotion_service(False, global_robot_real_no_zero_init_state_joint, global_robot_real_target_state_joint) # 查询规划状态

    # TODO: 因为目标为障碍物 所以把最终贴近目标的物体进行追踪
    rospy.loginfo(f"查询cumotion轨迹规划完成状态: {CUMOTION_FINSHED_FLAG}")

    # TODO: 调取/excute_plan_traj_cumotion发布Go轨迹服务
    GO_TRAJ_ID = 1
    call_execute_plan_traj(GO_TRAJ_ID)

    # TODO: 调取规划回来的贝塞尔轨迹加入到线程当中
    ...
    if not USE_SIM_FLAG:
        # 贝塞尔插值到最终目标位置
        call_bizer_plan_service(True, global_robot_new_real_target_state_joint, global_robot_real_current_state_joint) 
        # log 
        rospy.loginfo(" ---- 贝塞尔插值规划 call_bizer_plan_service(True, global_robot_new_real_target_state_joint, global_robot_real_current_state_joint)  -------------- ")
        rospy.loginfo(f"global_robot_new_real_target_state_joint.position: {global_robot_new_real_target_state_joint.position}")
    ...
    rospy.loginfo( " --------- 递水 | 往前递到指定位置 | call_bizer_plan_service(True, GoAndcatchStatemsg, SendJoinStatemsg) --- " )
    rospy.loginfo(f"global_robot_real_target_state_joint.position: {global_robot_real_target_state_joint.position}")
    ...
    rospy.loginfo( " --------- 从递水位置 | 回到避障的到达的位置 |   call_bizer_plan_service(True, SendWaterStatemsg, BackJoinStatemsg)  --- " )
    ...
    # TODO: 调取/excute_plan_traj_cumotion发布Back轨迹服务
    BACK_TRAJ_ID = 2
    call_execute_plan_traj(BACK_TRAJ_ID)
...
def start_grasp_process():
    """
        启动抓取流程。
    """
    global OBJECT_DETECTED_FLAG
    global GRASP_IN_PROGRESS_FLAG
    global IF_FIRST_GRASP_FLAG
    global GRASP_COUNT

    GRASP_IN_PROGRESS_FLAG = False  # 是否正在执行抓取流程
    if OBJECT_DETECTED_FLAG and not GRASP_IN_PROGRESS_FLAG:
        GRASP_IN_PROGRESS_FLAG = True  # 防止重复启动抓取
        rospy.loginfo("开始执行抓取流程...")
        try:
            if IF_FIRST_GRASP_FLAG:
                call_collect_service()
                time.sleep(5)
                IF_FIRST_GRASP_FLAG = False
            
            gendexgrasp()  # 调用你的抓取函数
            GRASP_COUNT += 1  # 增加抓取计数器

            # 每三次抓取调用一次 call_collect_service
            if GRASP_COUNT % 3 == 0:
                rospy.loginfo(f"抓取次数达到 {GRASP_COUNT}，调用 call_collect_service。")
                call_collect_service()
        except Exception as e:
            rospy.logerr(f"抓取过程中出现错误: {e}")
        finally:
            GRASP_IN_PROGRESS_FLAG = False  # 抓取完成后重置标志
    else:
        rospy.logwarn("物体没有检测到或抓取流程已经在进行中。")

# 定时器，用于定期检查物体是否检测到，并触发抓取
rospy.Timer(rospy.Duration(1), lambda event: start_grasp_process())  # 每秒检查一次物体检测状态
