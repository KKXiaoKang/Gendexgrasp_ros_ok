...
MAX_RIGHT_JOINT_ANGLE = [10.0, -18.0]
MAX_LEFT_JOINT_ANGLE = [-10.0, -18.0]
PRE_JOINT_AGNGLE = [0.0, -16.0]
...
class RobotCameraWatchAround:
    ...
    def publish_joint_data(self, event):
        ...
        # 切换 joint_data 的内容
        if self.switch:
            joint_data = MAX_RIGHT_JOINT_ANGLE  # 第一组数据
        else:
            joint_data = MAX_LEFT_JOINT_ANGLE  # 第二组数据
        
        # 每完成一个周期，增加周期计数
        if joint_data == MAX_LEFT_JOINT_ANGLE:
            self.cycle_count += 1
