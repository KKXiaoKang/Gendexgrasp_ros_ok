...
def call_bizer_plan_service(start_flag, joint_sapce_init, joint_sapce_target):
    ...
    bizer_plan_service_state_joint.position[:7] = joint_sapce_init.position # 左手位置赋值
...
def gendexgrasp():
    ...
    # （4） 根据监听/kuavo_cumotion_avoid_joint_target', JointState的结果 | 和手臂此时状态的结果 | 构建cumotion避障轨迹客户端 | 
    time.sleep(1)
    call_cumotion_service(True, global_robot_real_init_state_joint, global_robot_real_target_state_joint) # 第一次调用规划状态
    # （5） 如果成功调用/gendex_grasp_service 停止生成姿态 | 并且打印ik序号 | 并且按时用户是否继续 
    time.sleep(0.5)
    ...
    if not USE_SIM_FLAG:
        # 贝塞尔插值到最终目标位置
        call_bizer_plan_service(True, global_robot_real_init_state_joint, global_robot_real_current_state_joint) 
    ...
    """
    # （7）完成抓取姿态 | 关闭ik监听
    SendJoinStatemsg = global_robot_real_current_state_joint
    SendJoinStatemsg.position[0] -= 0.7 # 大臂pitch 抬高 | 弧度制
    SendJoinStatemsg.position[3] += 0.4 # 小臂pitch 降低 | 弧度制
    call_bizer_plan_service(True, global_robot_real_init_state_joint,  SendJoinStatemsg) 
    time.sleep(2)
    ...
    """
        从递水位置 | 回到避障的到达的位置
    """
    BackJoinStatemsg = global_robot_real_current_state_joint
    BackJoinStatemsg.position[:7] = global_robot_real_target_state_joint.position # 左手位置赋值
    call_bizer_plan_service(True, global_robot_real_init_state_joint, BackJoinStatemsg) 
    time.sleep(2)
    ...
    time.sleep(1)
    call_cumotion_service(True, global_robot_real_target_state_joint, global_robot_real_zero_state_joint)
    time.sleep(0.5)
    CUMOTION_FINSHED_FLAG = False # 规划未完成
    while not CUMOTION_FINSHED_FLAG:
        time.sleep(0.5)
